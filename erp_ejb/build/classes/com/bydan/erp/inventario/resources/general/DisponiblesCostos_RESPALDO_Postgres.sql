select 
	...
	(select sum(case 
					when df.id_tipo_definicion=1 or df.id_tipo_definicion=2 then d.cantidad
			 		when df.id_tipo_definicion=3 or df.id_tipo_definicion=4 then -d.cantidad
			 	else 0
		    	end) 
				...
				where d.id_bodega=bp.id_bodega and d.id_producto=bp.id_producto and
