...
public class Funciones2 {	
	...
	public static int GetAnioMesDiaFromDate(Date date,String sTipo) {
		Calendar cal = Calendar.getInstance();
	    cal.setTime(date);
	    Integer anio_mes_dia =0;
	    	    
	    if(sTipo.equals("A")) {
	    	anio_mes_dia =cal.get(Calendar.YEAR);
	    	
	    } else if(sTipo.equals("M")) {
	    	//LOS MESES EMPIEZAN DESDE 0
	    	anio_mes_dia =cal.get(Calendar.MONTH);
	    	anio_mes_dia=anio_mes_dia+1;
	    	
	    } else if(sTipo.equals("D")) {
	    	anio_mes_dia =cal.get(Calendar.DAY_OF_MONTH);
	    }
	    
	    return anio_mes_dia;
	}
	
	public static Integer GetUltimoDiaMes(Date today) {  
