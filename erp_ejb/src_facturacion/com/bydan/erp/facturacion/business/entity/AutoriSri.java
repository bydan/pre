...
public class AutoriSri extends AutoriSriAdditional implements Serializable ,Cloneable {//AutoriSriAdditional,GeneralEntity
	...
	private AutoriSri autorisriOriginal;
	
	private Map<String, Object> mapAutoriSri;
			
	public Map<String, Object> getMapAutoriSri() {
		return mapAutoriSri;
	}

	public void setMapAutoriSri(Map<String, Object> mapAutoriSri) {
		this.mapAutoriSri = mapAutoriSri;
	}
	
	public void inicializarMapAutoriSri() {
		this.mapAutoriSri = new HashMap<String,Object>();
	}
	
	public void setMapAutoriSriValue(String sKey,Object oValue) {
		this.mapAutoriSri.put(sKey, oValue);
	}
	
	public Object getMapAutoriSriValue(String sKey) {
		return this.mapAutoriSri.get(sKey);
	}
	...
	@Pattern(regex=AutoriSriConstantesFunciones.SREGEXNUMERO_SECUENCIAL,message=AutoriSriConstantesFunciones.SMENSAJEREGEXNUMERO_SECUENCIAL)
	private String numero_secuencial;
	
	@NotNull(message=ConstantesValidacion.SVALIDACIONNOTNULL)
	@Length(min=0,max=50,message=ConstantesValidacion.SVALIDACIONLENGTH)
	@NotEmpty(message=ConstantesValidacion.SVALIDACIONNOVACIO)
	@Pattern(regex=AutoriSriConstantesFunciones.SREGEXNUMERO_AUTOR,message=AutoriSriConstantesFunciones.SMENSAJEREGEXNUMERO_AUTOR)
	...
	public AutoriSri () throws Exception {
		super();		
    ...
    //PARA REPORTES
	public AutoriSri (Long id,Date versionRow,Long id_empresa,Long id_sucursal,Long id_tipo_documento_general,String numero_secuencial,String numero_autor,Date fecha_inicio,Date fecha_fin,Boolean esta_activo) throws Exception {
		super();		
	 ...
	 //PARA REPORTES
	public AutoriSri (Long id_empresa,Long id_sucursal,Long id_tipo_documento_general,String numero_secuencial,String numero_autor,Date fecha_inicio,Date fecha_fin,Boolean esta_activo) throws Exception {
		super();		
	...
	public boolean equals(Object object) {
		...
		if(object!=null) {
			autorisriLocal=(AutoriSri)object;
			
			if(autorisriLocal!=null) {
				if(this.getId()!=null && autorisriLocal.getId()!=null) {
					if(this.getId().equals(autorisriLocal.getId())) {
						equal=true;
					}
				}
			}
	...
	public String toString() {
		...
		if(!AutoriSriConstantesFunciones.CON_DESCRIPCION_DETALLADO) {
        	sDetalle=AutoriSriConstantesFunciones.getAutoriSriDescripcion(this);
		} else {
			sDetalle=AutoriSriConstantesFunciones.getAutoriSriDescripcionDetallado(this);
		}
		
		return sDetalle;
    }
	
	public AutoriSri getAutoriSriOriginal() {
		return this.autorisriOriginal;
	}
	
	public void setAutoriSriOriginal(AutoriSri autorisri) {
		try {
			this.autorisriOriginal=autorisri;
		} catch(Exception e) {
			;
		}
	}
	...
	protected AutoriSriAdditional autorisriAdditional=null;
	
	public AutoriSriAdditional getAutoriSriAdditional() {
		return this.autorisriAdditional;
	}
	
	public void setAutoriSriAdditional(AutoriSriAdditional autorisriAdditional) {
		try {
			this.autorisriAdditional=autorisriAdditional;
		} catch(Exception e) {
			;
		}
	}
		...
		try {
			if(this.esta_activo!=newesta_activo) {
				if(newesta_activo==null) {
					...
					if(Constantes.ISDEVELOPING) {
						System.out.println("AutoriSri:Valor nulo no permitido en columna esta_activo");
					}
				}
