...
public class EstadoCivil extends EstadoCivilAdditional implements Serializable ,Cloneable {//EstadoCivilAdditional,GeneralEntity
	...
	private EstadoCivil estadocivilOriginal;
	
	private Map<String, Object> mapEstadoCivil;
			
	public Map<String, Object> getMapEstadoCivil() {
		return mapEstadoCivil;
	}

	public void setMapEstadoCivil(Map<String, Object> mapEstadoCivil) {
		this.mapEstadoCivil = mapEstadoCivil;
	}
	
	public void inicializarMapEstadoCivil() {
		this.mapEstadoCivil = new HashMap<String,Object>();
	}
	
	public void setMapEstadoCivilValue(String sKey,Object oValue) {
		this.mapEstadoCivil.put(sKey, oValue);
	}
	
	public Object getMapEstadoCivilValue(String sKey) {
		return this.mapEstadoCivil.get(sKey);
	}
	...
	@Pattern(regex=EstadoCivilConstantesFunciones.SREGEXCODIGO,message=EstadoCivilConstantesFunciones.SMENSAJEREGEXCODIGO)
	private String codigo;
	
	@NotNull(message=ConstantesValidacion.SVALIDACIONNOTNULL)
	@Length(min=0,max=100,message=ConstantesValidacion.SVALIDACIONLENGTH)
	@NotEmpty(message=ConstantesValidacion.SVALIDACIONNOVACIO)
	@Pattern(regex=EstadoCivilConstantesFunciones.SREGEXNOMBRE,message=EstadoCivilConstantesFunciones.SMENSAJEREGEXNOMBRE)
	...
	public EstadoCivil () throws Exception {
		super();		
    ...
    //PARA REPORTES
	public EstadoCivil (Long id,Date versionRow,String codigo,String nombre) throws Exception {
		super();		
	 ...
	 //PARA REPORTES
	public EstadoCivil (String codigo,String nombre) throws Exception {
		super();		
	...
	public boolean equals(Object object) {
		...
		if(object!=null) {
			estadocivilLocal=(EstadoCivil)object;
			
			if(estadocivilLocal!=null) {
				if(this.getId()!=null && estadocivilLocal.getId()!=null) {
					if(this.getId().equals(estadocivilLocal.getId())) {
						equal=true;
					}
				}
			}
	...
	public String toString() {
		...
		if(!EstadoCivilConstantesFunciones.CON_DESCRIPCION_DETALLADO) {
        	sDetalle=EstadoCivilConstantesFunciones.getEstadoCivilDescripcion(this);
		} else {
			sDetalle=EstadoCivilConstantesFunciones.getEstadoCivilDescripcionDetallado(this);
		}
		
		return sDetalle;
    }
	
	public EstadoCivil getEstadoCivilOriginal() {
		return this.estadocivilOriginal;
	}
	
	public void setEstadoCivilOriginal(EstadoCivil estadocivil) {
		try {
			this.estadocivilOriginal=estadocivil;
		} catch(Exception e) {
			;
		}
	}
	...
	protected EstadoCivilAdditional estadocivilAdditional=null;
	
	public EstadoCivilAdditional getEstadoCivilAdditional() {
		return this.estadocivilAdditional;
	}
	
	public void setEstadoCivilAdditional(EstadoCivilAdditional estadocivilAdditional) {
		try {
			this.estadocivilAdditional=estadocivilAdditional;
		} catch(Exception e) {
			;
		}
	}
			...
			if(this.codigo!=newcodigo) {
				if(newcodigo==null) {
					...
					if(Constantes.ISDEVELOPING) {
						System.out.println("EstadoCivil:Valor nulo no permitido en columna codigo");
					}
				}

				if(newcodigo!=null&&newcodigo.length()>50) {
					newcodigo=newcodigo.substring(0,48);
					System.out.println("EstadoCivil:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=50 en columna codigo");
				}
		...
		try {
			if(this.nombre!=newnombre) {
				if(newnombre==null) {
					//newnombre="";
					if(Constantes.ISDEVELOPING) {
						System.out.println("EstadoCivil:Valor nulo no permitido en columna nombre");
					}
				}

				if(newnombre!=null&&newnombre.length()>100) {
					newnombre=newnombre.substring(0,98);
					System.out.println("EstadoCivil:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=100 en columna nombre");
				}
