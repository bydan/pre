...
public class CajaEgreso extends CajaEgresoAdditional implements Serializable ,Cloneable {//CajaEgresoAdditional,GeneralEntity
	...
	private CajaEgreso cajaegresoOriginal;
	
	private Map<String, Object> mapCajaEgreso;
			
	public Map<String, Object> getMapCajaEgreso() {
		return mapCajaEgreso;
	}

	public void setMapCajaEgreso(Map<String, Object> mapCajaEgreso) {
		this.mapCajaEgreso = mapCajaEgreso;
	}
	
	public void inicializarMapCajaEgreso() {
		this.mapCajaEgreso = new HashMap<String,Object>();
	}
	
	public void setMapCajaEgresoValue(String sKey,Object oValue) {
		this.mapCajaEgreso.put(sKey, oValue);
	}
	
	public Object getMapCajaEgresoValue(String sKey) {
		return this.mapCajaEgreso.get(sKey);
	}
	...
	@Pattern(regex=CajaEgresoConstantesFunciones.SREGEXSECUENCIAL,message=CajaEgresoConstantesFunciones.SMENSAJEREGEXSECUENCIAL)
	...
	public CajaEgreso () throws Exception {
		super();		
    ...
    //PARA REPORTES
	public CajaEgreso (Long id,Date versionRow,Long id_empresa,Long id_sucursal,Long id_usuario,Long id_caja,Long id_turno_pun_ven,String secuencial,Date fecha,Time hora,Double valor) throws Exception {
		super();		
	 ...
	 //PARA REPORTES
	public CajaEgreso (Long id_empresa,Long id_sucursal,Long id_usuario,Long id_caja,Long id_turno_pun_ven,String secuencial,Date fecha,Time hora,Double valor) throws Exception {
		super();		
	...
	public boolean equals(Object object) {
		...
		if(object!=null) {
			cajaegresoLocal=(CajaEgreso)object;
			
			if(cajaegresoLocal!=null) {
				if(this.getId()!=null && cajaegresoLocal.getId()!=null) {
					if(this.getId().equals(cajaegresoLocal.getId())) {
						equal=true;
					}
				}
			}
	...
	public String toString() {
		...
		if(!CajaEgresoConstantesFunciones.CON_DESCRIPCION_DETALLADO) {
        	sDetalle=CajaEgresoConstantesFunciones.getCajaEgresoDescripcion(this);
		} else {
			sDetalle=CajaEgresoConstantesFunciones.getCajaEgresoDescripcionDetallado(this);
		}
		
		return sDetalle;
    }
	
	public CajaEgreso getCajaEgresoOriginal() {
		return this.cajaegresoOriginal;
	}
	
	public void setCajaEgresoOriginal(CajaEgreso cajaegreso) {
		try {
			this.cajaegresoOriginal=cajaegreso;
		} catch(Exception e) {
			;
		}
	}
	...
	protected CajaEgresoAdditional cajaegresoAdditional=null;
	
	public CajaEgresoAdditional getCajaEgresoAdditional() {
		return this.cajaegresoAdditional;
	}
	
	public void setCajaEgresoAdditional(CajaEgresoAdditional cajaegresoAdditional) {
		try {
			this.cajaegresoAdditional=cajaegresoAdditional;
		} catch(Exception e) {
			;
		}
	}
		...
		try {
			if(this.secuencial!=newsecuencial) {
				if(newsecuencial==null) {
					//newsecuencial="";
					if(Constantes.ISDEVELOPING) {
						System.out.println("CajaEgreso:Valor nulo no permitido en columna secuencial");
					}
				}

				if(newsecuencial!=null&&newsecuencial.length()>50) {
					newsecuencial=newsecuencial.substring(0,48);
					System.out.println("CajaEgreso:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=50 en columna secuencial");
				}
		...
		try {
			if(this.fecha!=newfecha) {
				if(newfecha==null) {
					//newfecha=new Date();
					if(Constantes.ISDEVELOPING) {
						System.out.println("CajaEgreso:Valor nulo no permitido en columna fecha");
					}
				}
		...
		try {
			if(this.hora!=newhora) {
				if(newhora==null) {
					...
					if(Constantes.ISDEVELOPING) {
						System.out.println("CajaEgreso:Valor nulo no permitido en columna hora");
					}
				}
		...
		try {
			if(this.valor!=newvalor) {
				if(newvalor==null) {
					//newvalor=0.0;
					if(Constantes.ISDEVELOPING) {
						System.out.println("CajaEgreso:Valor nulo no permitido en columna valor");
					}
				}
